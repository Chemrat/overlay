...
src_prepare() {
	...
	cp "${S}"/categories/${res}/applications-system.png \
		"${S}"/actions/${res}/application-menu.png || die
	cp "${S}"/categories/${res}/applications-system.png \
		"${S}"/actions/${res}/applications-other.png || die
	done
	cp "${S}"/places/scalable/start-here-gentoo.svg \
		"${S}"/places/scalable/start-here.svg || die
	cp "${S}"/categories/scalable/applications-system.svg \
		"${S}"/actions/scalable/application-menu.svg || die
	cp "${S}"/categories/scalable/applications-system.svg \
		"${S}"/actions/scalable/applications-other.svg || die
}
